...
export function RankingCard({ ranking, rank }: RankingCardProps) {
    ...
    const totalHomeworks = ranking.mbtiProgress.length + ranking.poseProgress.length;
    const submittedCount = [...ranking.mbtiProgress, ...ranking.poseProgress].filter(
        (s) => s === true || (s as any) === "TRUE"
    ).length;
    const progressPercent = (submittedCount / totalHomeworks) * 100;
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex gap-1">
                            {ranking.mbtiProgress.map((submitted, i) => (
                                <div 
                                    key={i}
                                    title={`MBTI ${i}주차: ${submitted ? '제출' : '미제출'}`}
