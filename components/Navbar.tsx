    ...
    <>
      <nav ref={navRef} className="mt-6 mx-auto w-[95%] max-w-5xl z-40 relative">
        <div className="glass rounded-[2rem] px-8 py-4 flex items-center justify-between border-white/20 shadow-2xl">
          ...
          <ul className="hidden lg:flex items-center gap-2 p-1 bg-secondary/50 rounded-2xl border border-white/5">
            {navItems.map((item) => (
              <li key={item.path}>
                <Link
                  href={item.path}
                  className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 whitespace-nowrap ${pathname === item.path
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={`lg:hidden min-w-[60px] h-9 rounded-full flex items-center justify-center px-3 border transition-all ${isMobileMenuOpen
        ...
        {isMounted && isMobileMenuOpen && (
          <div className="lg:hidden mt-3 glass rounded-[1.75rem] border border-white/10 p-3 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
