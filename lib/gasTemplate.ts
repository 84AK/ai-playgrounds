...
"/* ==============================================================",
"   AI 연구소 통합 백엔드 API V9.8.5_Master_Unfiltered_Final",
"   (모든 기능 100% 복구 + 쇼케이스 전수 공개 최적화 패치)",
"   ============================================================== */",
"",
...
"      if (sheet.getLastRow() === 0) sheet.appendRow([\"Timestamp\", \"Author\", \"Title\", \"Description\", \"Url\", \"Password\", \"category\", \"visible\"]);",
"      sheet.appendRow([",
"        getKoreanTime(), ",
...
"  const ss = SpreadsheetApp.getActiveSpreadsheet();",
"  try {",
"    if (action === 'testConnection') return createJSONResponse({ status: \"success\", message: \"연결 성공 (V9.8.5 Master Final)\", spreadsheetName: ss.getName() });",
"",
...
"      } catch (err) { return createJSONResponse({ status: \"error\", message: \"FileNotFound\" }); }",
"    }",
"",
"    if (action === 'getUser') {",
"      const sheet = ss.getSheetByName(SHEET_USERS);",
...
"      for (let i = 1; i < rows.length; i++) if (rows[i][0] == uid) return createJSONResponse({ status: \"success\", data: { name: rows[i][0], school: rows[i][1], password: rows[i][2], avatar: rows[i][3], grade: rows[i][5], classGroup: rows[i][6] } });",
...
"      const rows = sheet.getDataRange().getDisplayValues();",
"      let data = []; ",
"      for (let i=1; i<rows.length; i++) data.push({ user_id: rows[i][1], author: rows[i][1], title: rows[i][2], description: rows[i][3], url: rows[i][4], timestamp: rows[i][0] });",
"      return createJSONResponse({ status: \"success\", data: data });",
"    }",
